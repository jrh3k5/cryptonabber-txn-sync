...
var _ = Describe("TransfersFromEtherscanCSV", func() {
	...
	DescribeTable("missing required column", func(missingColumn string) {
		...
		Expect(toRemoveIndex).ToNot(Equal(-1), "column '%s' to remove should be found", missingColumn)
		...
		_, err := transactionpkg.TransfersFromEtherscanCSV(context.Background(), usdcDetails, &buffer)
		Expect(err).To(MatchError(ContainSubstring(fmt.Sprintf("CSV is missing required column: %s", missingColumn))), "parsing CSV with missing column '%s' should fail", missingColumn)
	},
